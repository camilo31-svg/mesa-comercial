...
export function detectObjections(text){
 ...
 if(/no (me )?(llam|contact)|no quiero (el seguro|contratar)|no me interesa|borr(a|e).*datos/.test(t))return [{id:"stop",title:"Rechazo explícito",say:"Entendido. Gracias por decírmelo. Dejamos aquí la propuesta y respetamos tu petición.",ask:"",close:"No insistir. Registra la petición y tramítala en el sistema autorizado.",follow:"No hagas otra llamada comercial si ha pedido no ser contactado."}];
 ...
 if(/\b(no|nunca) (me )?(llames|contactes)\b/.test(t))return [{id:"stop",title:"Rechazo explícito",say:"Entendido. No volveremos a contactarte con esta propuesta.",ask:"",close:"Registra la petición en el sistema autorizado.",follow:"No hacer seguimiento comercial."}];
 if(/\b(no me presiones|me estas presionando|deja de insistir|no insistas)\b/.test(t))return [special.pressure];
 if(/\b(iban|numero de cuenta|numero de tarjeta|mi dni|mi nif)\b/.test(t))return [special.privacy];
 if(/\b(me garantizas|puedes garantizar|garantizame|me aseguras que)\b/.test(t))return [special.guarantee];
 if(/^(si|vale|de acuerdo|adelante|contratemos|quiero contratar|hagamoslo|me interesa contratar)[.! ]*$/.test(t)||/\b(quiero contratar|contratemos|iniciemos la solicitud)\b/.test(t))return [special.ready];
 ...
 if(/\b(no lo necesito|no necesito otro seguro|no veo (la )?necesidad|estoy bien como estoy|no tengo ningun problema)\b/.test(t))return [special.noNeed];
 const scores=objections.map(o=>({o,score:o.words.reduce((n,w)=>n+(includesPhrase(t,w)&&!(o.id==="precio"&&/\bno (es|me parece|lo veo) caro\b/.test(t))?w.length:0),0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);
 return scores.map(x=>x.o).slice(0,3);
}
export function scriptFor(text,selectedId,mode,context={}){
 ...
 return {...o,say,ask,matches,contextLine:need?"Tu prioridad: "+need:"Aún falta identificar la necesidad principal.",proposal:product?"Propuesta en revisión: "+product:"Selecciona una propuesta después de conocer la necesidad."};
}
