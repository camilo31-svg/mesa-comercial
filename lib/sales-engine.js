...
export function scriptFor(text,selectedId,mode,context){
 ...
 const selected=matches[0]?.id==="stop"||matches[0]?.id==="exactitud"?matches[0]:objections.find(x=>x.id===selectedId)||matches[0];
 ...
 return {...o,say:mode==="follow"?o.follow:o.say,matches,contextLine:need?"Tu prioridad: "+need:"Aún falta identificar la necesidad principal.",proposal:product?"Propuesta en revisión: "+product:"Selecciona una propuesta después de conocer la necesidad."};
}
